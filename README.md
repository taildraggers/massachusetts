...
Daily aggregator of Massachusetts-located taildragger classified
...
[Maryland](https://github.com/taildraggers/maryland) repos' approach, with
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, MA` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Massachusetts"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/massachusetts/`
...
```html
<iframe
  src="https://taildraggers.github.io/massachusetts/"
  title="Taildragger Ads in Massachusetts"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_MASSACHUSETTS_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
