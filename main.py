"""Aggregates Massachusetts-located listings from every taildraggers.com
...
ones whose location ends in ", MA".
...
Indiana/Iowa/Kansas/Kentucky/Louisiana/Maine/Maryland repos' approach, with
the state filter swapped to "MA".
"""
...
PAGE_TITLE = "Taildragger Ads in Massachusetts"
STATE_ABBR = "MA"
...
_MASSACHUSETTS_LOCATION_RE = re.compile(r",\s*MA\s*$", re.IGNORECASE)


def _is_massachusetts(location: str) -> bool:
    return bool(_MASSACHUSETTS_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        massachusetts_rows = [row for row in rows if _is_massachusetts(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(massachusetts_rows)} in Massachusetts")
        all_listings.extend(massachusetts_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Massachusetts listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Massachusetts listings: {len(listings)}")
